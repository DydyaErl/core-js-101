...
function findFirstSingleChar(str) {
  ...
  // Поиск первого символа с count = 1
  for (const char of str) {
    if (charCount[char] === 1) {
      return char;
    }
...
function getCommonDirectoryPath(pathes) {
  if (pathes.length === 0) return ''; // Если пустой массив, вернуть пустую строку
  ...
  for (let i = 1; i < splitPaths.length; i++) {
    const minLength = Math.min(commonPath.length, splitPaths[i].length);


    let j = 0;
    while (j < minLength && commonPath[j] === splitPaths[i][j]) {
      j++;
    }
...
function getMatrixProduct(m1, m2) {
  ...
  // Создание результата с инициализацией нулями
  const result = Array.from({ length: rowsM1 }, () => Array(colsM2).fill(0));

  // Вычисление элементов результата
